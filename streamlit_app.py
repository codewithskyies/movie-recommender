...
REQUEST_TIMEOUT = 60   # Render cold start can take 50s
MAX_RETRIES     = 3
...
# ── Wake-up ping ──────────────────────────────────────────────
def wake_server():
    if st.session_state.server_awake:
        return True
    try:
    ...
    banner   = st.info("⏳ **Server is waking up** (Render free tier sleeps after inactivity). This takes ~30–50 s. Please wait…")
    progress = st.progress(0)
 
    for attempt in range(MAX_RETRIES):
        try:
            r = requests.get(f"{API_BASE}/health", timeout=70)
            if r.status_code == 200:
                banner.empty()
                progress.empty()
                st.session_state.server_awake = True
            ...
            pass
        progress.progress((attempt + 1) / MAX_RETRIES)
 
    banner.empty()
    progress.empty()
    st.error("❌ Server did not respond. Refresh the page in ~30 seconds.")
    return False
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if attempt < MAX_RETRIES - 1:
                st.warning(f"⏳ Slow response, retrying… ({attempt + 2}/{MAX_RETRIES})")
                time.sleep(2)
            else:
                return None, "Timed out. The server may still be waking up — wait 30 s and refresh."
...
st.markdown("<div class='small-muted'>Search → select → details + recommendations</div>", unsafe_allow_html=True)
st.divider()
 
# ── Wake server before any real work ──────────────────────────
if not wake_server():
    st.stop()
...
if st.session_state.view == "home":
    typed = st.text_input("Search by movie title", placeholder="Type: avenger, batman, love...")
    st.divider()
            ...
            if err or data is None:
                ...
                if suggestions:
                    ...
                    if selected != "-- Select a movie --":
                        goto_details({s[0]: s[1] for s in suggestions}[selected])
                else:
...
# ═════════════════════════════════════
    ...
    with right:
        ...
        st.markdown(f"<div class='small-muted'>Release: {release} &nbsp;|&nbsp; Genres: {genres}</div>", unsafe_allow_html=True)
        st.markdown("---")
    ...
    if title:
        ...
        if not err2 and bundle:
            ...
            poster_grid(bundle.get("genre_recommendations", []), cols=grid_cols, key_prefix="genre")
        else:
